...
from surrogates.svgp import filter_nan
from surrogates.svgp import BaseModel
from variable_domains.transforms import TorchMinMaxScaler, TorchStandardScaler

from surrogates.svgp import DummyFeatureExtractor, default_kern
from optimisation_oracles.sgld import SGLD
...
class GP(BaseModel):
    ...
    @staticmethod
    def _with_retried_cholesky(fn, max_jitter=10):
        jitter = 1e-8
        while True:
            with cholesky_jitter(double_value=jitter, float_value=100 * jitter, half_value=10000 * jitter):
                try:
                    return fn(), True
        ...
        with gpytorch.settings.debug(False):
            def _sample():
                if self.pred_likeli:
                    ...
                    pred = self.gp(Xc_t, Xe_t)
                return pred.rsample(torch.Size((n_samples,))).view(n_samples, Xc_t.shape[0], self.num_out)

            samp, success = self._with_retried_cholesky(_sample)
            if not success:
                return super().sample_y(Xc, Xe, n_samples)
