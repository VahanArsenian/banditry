...
from banditry.variable_domains.transforms import DummyFeatureExtractor, TorchMinMaxScaler, TorchStandardScaler
    ...
    def __init__(self, mean, kern, u, learn_u, use_ngd):
        ...
        if use_ngd:
            variational_distribution = (
                NaturalVariationalDistribution(num_inducing)
                if u.dtype in [torch.float32, torch.float64]
                else TrilNaturalVariationalDistribution(num_inducing)
            )
        else:
                    ...
                    kern=deepcopy(
                        conf.get(
                            ...
                            default_kern(
                                ...
                                y[:, i],
                                self.fe.total_dim,
                                conf.get("ard_kernel", True),
                                conf.get("product_kernel", True),
                            ),
                        )
                    ),
...
class SVGP(BaseModel):
    ...
    def fit(self, Xc: FloatTensor, Xe: LongTensor, y: FloatTensor):
        ...
        mll = [
            gpytorch.mlls.VariationalELBO(self.lik[i], self.gp.gp[i], num_data=y.shape[0], beta=self.beta)
            for i in range(self.num_out)
        ]
        for epoch in range(self.num_epochs):
            ...
            if self.verbose and ((epoch + 1) % self.print_every == 0 or epoch == 0):
                ...
                log.debug(f"After {epoch + 1} epochs, loss = {epoch_loss:g}")
        self.gp.eval()
        self.lik.eval()
